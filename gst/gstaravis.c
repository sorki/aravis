...
gst_aravis_init (GstAravis *gst_aravis)
{
	...
	gst_aravis->user_output_value = FALSE;
	gst_aravis->gamma = 1;
			 ...
			 const GValue * value, GParamSpec * pspec)
{
	...
	switch (prop_id) {
		...
		case PROP_USER_OUTPUT_VALUE:
			GST_OBJECT_LOCK (gst_aravis);
			gst_aravis->user_output_value = g_value_get_boolean (value);

			GError *error = NULL;
			if (gst_aravis->camera != NULL)
				arv_camera_set_boolean (gst_aravis->camera, "UserOutputValue", gst_aravis->user_output_value, &error);
			 ...
			 GParamSpec * pspec)
{
	...
	switch (prop_id) {
		...
		case PROP_USER_OUTPUT_VALUE:
			GST_OBJECT_LOCK (gst_aravis);
			g_value_set_boolean (value, gst_aravis->user_output_value);
			GST_OBJECT_UNLOCK (gst_aravis);
			break;
		case PROP_GAMMA:
			GST_OBJECT_LOCK (gst_aravis);
...
gst_aravis_class_init (GstAravisClass * klass)
{
	...
	properties[PROP_USER_OUTPUT_VALUE] =
		g_param_spec_boolean ("user-output-value",
				  "User output value",
